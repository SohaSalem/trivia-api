...
def create_app(test_config=None):
  ...
  def get_quizzes():
      ...
      if(len(prev_question) > 0):
        if(category_id > 0):
          ...
          .filter(~Question.id.in_(prev_question))\
          .order_by(func.random()).first()
        else:
          current_question = Question.query.filter(~Question.id.in_(prev_question)).order_by(func.random()).first()
      else:
        if(category_id>0):
          current_question = Question.query.filter(
  ...
  def bad_request_err_handler(error):
    return jsonify({
      'success': False,
      'error': 400,
      'message': 'bad request'
